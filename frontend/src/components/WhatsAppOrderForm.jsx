...
import { AlertCircle, CheckCircle2, MapPin, Phone, UserRound } from "lucide-react";
...
export default function WhatsAppOrderForm({ product, desktop = false }) {
  ...
  const stock = Number(selectedVariant?.stock ?? -1);
  ...
  function submitOrder(event) {
    ...
    if (isOutOfStock) {
      alert("هذا اللون غير متوفر حاليًا. اختر لونًا آخر أو عدل المخزون من لوحة التحكم.");
      return;
    }
    ...
    const message = [
      ...
      "",
      `المنتج: ${product.name}`,
      selectedVariant ? `اللون: ${selectedVariant.name}` : null,
      `السعر: ${formatPrice(product.price, product.currency)}`,
      selectedVariant && Number.isFinite(stock) && stock >= 0 ? `المخزون الحالي: ${stock}` : null,
      `الاسم: ${form.fullName}`,
      `الهاتف: ${form.phone}`,
      `الولاية: ${form.wilaya}`,
  ...
  return (
    <form onSubmit={submitOrder} className={desktop ? "space-y-4" : "space-y-4 px-3 pb-2 pt-2 min-[360px]:px-4 sm:px-6"}>
      <section className={desktop ? "rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm" : "store-card p-5"}>
        ...
        {selectedVariant ? (
          <div className={`mb-4 rounded-2xl border px-4 py-3 text-sm font-black ${isOutOfStock ? "border-amber-200 bg-amber-50 text-amber-800" : "border-emerald-100 bg-emerald-50 text-emerald-800"}`}>
            <div className="flex items-center gap-2">
              <span className="inline-block h-4 w-4 rounded-full border border-white shadow" style={{ backgroundColor: selectedVariant.colorHex }} />
              اللون المختار {selectedVariant.name}
            </div>
            <div className="mt-1 text-xs font-bold">
              {isOutOfStock ? "هذا اللون غير متوفر حاليًا" : `السعر الحالي ${formatPrice(product.price, product.currency)}`}
            </div>
          </div>
        ) : null}
        ...
        {isOutOfStock ? (
          <div className="mt-4 flex items-center gap-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-black text-amber-800">
            <AlertCircle size={17} /> لا يمكن إرسال الطلب لهذا اللون لأن المخزون يساوي صفر
          </div>
        ) : null}

        {desktop ? (
          <button type="submit" disabled={isOutOfStock} className="btn-whatsapp mt-5 w-full text-base disabled:cursor-not-allowed disabled:opacity-60">
      ...
      {!desktop ? (
        <button type="submit" disabled={isOutOfStock} className="btn-whatsapp fixed bottom-3 left-1/2 z-40 w-[calc(100%-24px)] max-w-[616px] -translate-x-1/2 text-base disabled:cursor-not-allowed disabled:opacity-60">
